...
export default function EditHealthRecordPage() {
  ...
  useEffect(() => {
    if (token && patientId && recordId) {
      Promise.all([
        nurseApi.getIbuHamilDetail(token, patientId),
        healthRecordsApi.getHealthRecordDetail(token, recordId)
      ])
        .then(([patientData, recordData]) => {
          ...
          reset({
            checkup_date: recordData.checkup_date.split('T')[0],
            gestational_age_weeks: recordData.gestational_age_weeks,
            gestational_age_days: recordData.gestational_age_days,
            ...
            body_temperature: recordData.body_temperature,
            blood_glucose: recordData.blood_glucose,
            hemoglobin: recordData.hemoglobin,
            upper_arm_circumference: recordData.upper_arm_circumference,
            fundal_height: recordData.fundal_height,
            fetal_heart_rate: recordData.fetal_heart_rate,
            protein_urin: recordData.protein_urin,
  ...
  }, [token, patientId, recordId, reset, router]);
  ...
  const onSubmit = async (data: HealthRecordFormData) => {
    ...
    try {
      // For update, we only send the fields that changed + required IDs if needed, 
      // but usually standard PUT replaces or patches. 
      // The API expects the payload structure defined in types.
      const payload = {
        ...data,
      ...
      console.log("🚀 [DEBUG] Submitting Edit Payload:", {
      ...
      await healthRecordsApi.updateHealthRecord(token, recordId, payload);
      router.push(`/perawat/pasien/${patientId}`);
    } catch (error) {
      console.error("Update failed", error);
      alert("Gagal memperbarui data pemeriksaan.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onInvalid = (errors: any) => {
    ...
    alert("Terdapat data yang tidak valid pada form. Silakan cek kembali inputan Anda.");
  };
