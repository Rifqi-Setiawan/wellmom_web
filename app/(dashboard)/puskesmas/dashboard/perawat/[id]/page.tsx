...
export default function NurseDetailPage() {
  ...
  const nurseId = typeof idParam === 'string' ? parseInt(idParam, 10) : Number(idParam);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedNurses: HealthPersonnel[] = nursesResponse.perawat_list.map((n: any) => {
        const colors = [
      ...
      console.log('🔎 Looking for nurse with ID:', nurseId, 'Type:', typeof nurseId);
      ...
      const currentNurse = mappedNurses.find(n => {
        ...
        console.log(`  Comparing: n.id=${n.id} (${typeof n.id}) === nurseId=${nurseId} (${typeof nurseId}) = ${match}`);
        return match;
      });
      ...
      if (currentNurse) {
        ...
        try {
          const patientsData = await nurseApi.getPatientsByNurse(token, nurseId);
        ...
        console.error('❌ Nurse not found! Available IDs:', mappedNurses.map(n => n.id));
        setNurse(null);
      }
    } catch (error) {
  ...
  const getRiskScore = (patient: IbuHamil): 'Low Risk' | 'Medium Risk' | 'High Risk' => {
  ...
  const handleTransferPatient = async (
    ...
    await nurseApi.transferSinglePatient(token, sourcePerawatId, ibuHamilId, targetPerawatId);
    ...
    return (
      <div className="p-8">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          <p className="text-lg font-semibold text-red-900 mb-2">Perawat tidak ditemukan</p>
    ...
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 min-w-[180px]">
          ...
          <p className="text-3xl font-bold text-[#3B9ECF]">
            {patients.length} <span className="text-base font-normal text-gray-600">Ibu Hamil</span>
          </p>
        </div>
      </div>
        ...
        {filteredPatients.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <p className="text-sm text-gray-600">
              ...
              {Math.min(currentPage * itemsPerPage, filteredPatients.length)} dari{' '}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-blue-900 mb-1">Petunjuk:</p>
            <p className="text-sm text-blue-800">
              Klik tombol &quot;Pindahkan Pasien&quot; untuk mendelegasikan pendampingan ibu hamil kepada tenaga kesehatan lain yang tersedia. Hal ini biasanya dilakukan jika nakes bersangkutan sedang cuti atau beban kerja melebihi kapasitas.
            </p>
          </div>
        </div>
      </div>
      ...
      <TransferSinglePatientModal
        ...
        sourceNurse={nurse}
        targetNurses={allNurses.filter(n => n.id !== nurseId && n.status === 'Aktif')}
